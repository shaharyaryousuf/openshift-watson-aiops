...
wget ${RHCOS_BASEURL}/4.5/4.5.6/rhcos-4.5.6-x86_64-installer-initramfs.x86_64.img	
wget ${RHCOS_BASEURL}/4.5/4.5.6/rhcos-4.5.6-x86_64-installer-kernel-x86_64
wget ${RHCOS_BASEURL}/4.5/4.5.6/rhcos-4.5.6-x86_64-metal.x86_64.raw.gz
popd
...
# Apply configuration
cp -r bastion/* /
cp -r ~/openshift-install-4/bastion/root/bashrc ~/.bashrc
source ~/.bashrc
...
OCP4_BASEURL=https://mirror.openshift.com/pub/openshift-v4/clients/ocp/4.5.6
curl -s ${OCP4_BASEURL}/openshift-install-linux-4.5.6.tar.gz | tar -xzf - -C /usr/local/bin/ openshift-install
curl -s ${OCP4_BASEURL}/openshift-client-linux-4.5.6.tar.gz	 | tar -xzf - -C /usr/local/bin/ oc